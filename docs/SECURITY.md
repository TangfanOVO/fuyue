...
- 可选接入码只在兑换请求正文中出现，成功后换成随机会话：同源优先使用 HttpOnly Cookie；Safari 跨站连接使用当前浏览器会话中的 bearer。bearer 不进 LocalData、URL 或导出包，但不具备 HttpOnly 隔离；错误尝试按来源地址做短时限流。生产多用户服务仍需正式账号与撤销机制。

...
function normalizedBaseUrl(value: string): string {
  ...
  if (!trimmed) throw new GatewayError("请先填写转接服务地址");
  let url: URL;
  try {
    url = new URL(trimmed);
  } catch {
    throw new GatewayError("转接服务地址不是有效网址");
  }
  const isLocal = ["localhost", "127.0.0.1", "[::1]"].includes(url.hostname);
  if (url.protocol !== "https:" && !(isLocal && url.protocol === "http:")) {
    throw new GatewayError("远程转接服务必须使用 HTTPS；只有本机 localhost 可以使用 HTTP");
  }
...
async function responseError(response: Response): Promise<GatewayError> {
  ...
  if (response.status === 401) return new GatewayError(detail || "转接服务需要重新连接", 401);
  if (response.status === 403) return new GatewayError(detail || "转接服务拒绝了这次请求，请检查接入权限", 403);
  if (response.status === 429) return new GatewayError(detail || "请求太频繁，请稍后再试", 429);
  return new GatewayError(detail || `转接服务返回错误 ${response.status}，请稍后重试`, response.status);
}
...
function validStatus(value: unknown): GatewayStatus {
  ...
  if (!item || typeof item.ok !== "boolean" || typeof item.service !== "string" || typeof item.activeProviderId !== "string" || !Array.isArray(providers)) throw new GatewayError("转接服务返回的运行状态不完整");
  const parsed = providers.map((provider) => {
    const entry = object(provider); const capabilities = entry?.capabilities;
    if (!entry || typeof entry.id !== "string" || typeof entry.label !== "string" || !Array.isArray(capabilities) || capabilities.some((capability) => !["chat", "vision", "tools", "audio"].includes(String(capability)))) throw new GatewayError("转接服务返回的聊天厂商信息不完整");
    const reasoningEfforts = entry.reasoningEfforts;
    const clientTools = entry.clientTools;
    if (reasoningEfforts !== undefined && (!Array.isArray(reasoningEfforts) || reasoningEfforts.some((effort) => !["auto", "none", "low", "medium", "high", "xhigh", "max"].includes(String(effort))))) throw new GatewayError("转接服务返回的思考深度选项不完整");
    if (clientTools !== undefined && (!Array.isArray(clientTools) || clientTools.some((name) => !isClientToolName(name)))) throw new GatewayError("转接服务返回的本机工具清单不完整");
  ...
  const parsedCapabilities: CapabilityStatus[] = Array.isArray(capabilities) ? capabilities.map((capability) => {
    ...
    if (!entry || typeof entry.id !== "string" || !["local", "custom_backend", "fuyue_service", "disabled"].includes(String(entry.mode)) || !["ready", "local_only", "surface_only", "needs_backend", "disabled", "error"].includes(String(entry.state))) throw new GatewayError("转接服务返回的功能状态不完整");
    return {
...
function validLife(value: unknown): LifeOverviewItem[] {
  if (!Array.isArray(value)) throw new GatewayError("转接服务返回的日程不是可读取的清单");
  return value.map((entry) => { const item = object(entry); if (!item || typeof item.id !== "string" || typeof item.title !== "string" || typeof item.startAt !== "string" || Number.isNaN(Date.parse(item.startAt))) throw new GatewayError("转接服务返回的某条日程不完整"); return { id: item.id, title: item.title, startAt: item.startAt, ...(typeof item.endAt === "string" ? { endAt: item.endAt } : {}), ...(typeof item.location === "string" ? { location: item.location } : {}), ...(typeof item.kind === "string" ? { kind: item.kind } : {}) }; });
}
function validMood(value: unknown): MoodSnapshot | null {
  if (value == null) return null; const item = object(value);
  if (!item || typeof item.title !== "string" || typeof item.detail !== "string" || typeof item.updatedAt !== "string" || Number.isNaN(Date.parse(item.updatedAt)) || typeof item.sourceLabel !== "string" || !item.sourceLabel.trim()) throw new GatewayError("转接服务返回的心情状态不完整");
...
function validCobrowseInspection(value: unknown): CobrowseInspection {
  ...
  if (!item || !["read", "blocked"].includes(String(item.status)) || typeof item.requestedUrl !== "string" || typeof item.finalUrl !== "string" || typeof item.sourceLabel !== "string" || typeof item.title !== "string" || typeof item.summary !== "string" || typeof item.detail !== "string") throw new GatewayError("转接服务没有返回完整的页面读取结果");
...
function validCobrowseComment(value: unknown): CobrowseComment {
  const item = object(value);
  if (!item || typeof item.comment !== "string" || !item.comment.trim() || typeof item.sourceLabel !== "string" || typeof item.modelLabel !== "string") throw new GatewayError("转接服务没有返回完整评论，这次不会保存空白内容");
...
function validVoiceStatus(value: unknown): VoiceStatus {
  ...
  if (!item || typeof item.ok !== "boolean" || typeof item.service !== "string" || typeof item.activeProviderId !== "string" || !Array.isArray(providers)) throw new GatewayError("转接服务返回的语音状态不完整");
  const parsed = providers.map((provider) => {
    const entry = object(provider);
    if (!entry || !["elevenlabs", "doubao", "custom"].includes(String(entry.id)) || typeof entry.label !== "string" || typeof entry.configured !== "boolean" || typeof entry.voice !== "string" || typeof entry.model !== "string") throw new GatewayError("转接服务返回的语音厂商信息不完整");
    return { id: entry.id as VoiceProviderId, label: entry.label, configured: entry.configured, voice: entry.voice, model: entry.model };
  });
  if (item.activeProviderId && !["elevenlabs", "doubao", "custom"].includes(item.activeProviderId)) throw new GatewayError("转接服务设置的默认语音厂商无法识别");
...
function validVoiceTranscript(value: unknown): VoiceTranscript {
  const item = object(value);
  if (!item || typeof item.text !== "string" || !item.text.trim() || !["elevenlabs", "doubao", "custom"].includes(String(item.providerId)) || typeof item.providerLabel !== "string") throw new GatewayError("语音服务没有返回完整转写，这一轮不会写入原文");
...
function validVoiceSynthesis(value: unknown): VoiceSynthesis {
  const item = object(value);
  if (!item || typeof item.audioBase64 !== "string" || item.audioBase64.length < 16 || !["audio/mpeg", "audio/wav"].includes(String(item.mediaType)) || !["elevenlabs", "doubao", "custom"].includes(String(item.providerId)) || typeof item.providerLabel !== "string") throw new GatewayError("语音服务没有返回可播放的声音，请重试");
...
function parseEvent(value: unknown): ChatGatewayEvent | null {
  ...
  if (event.type === "done") {
    ...
    if (!item
      ...
      })))) throw new GatewayError("转接服务没有完整说明这次回复是否完成");
    return {
      type: "done",
...
async function* parseEventStream(response: Response): AsyncGenerator<ChatGatewayEvent> {
  if (!response.body) throw new GatewayError("模型服务没有返回回复内容", response.status);
  ...
  while (true) {
    ...
    for (const rawLine of lines) {
      ...
      try {
        ...
        throw new GatewayError("模型服务返回了赴约暂时无法识别的内容", response.status);
      }
    }
    if (done) break;
  }
  if (buffer.trim()) {
    try {
      ...
      throw new GatewayError("模型服务的最后一段回复不完整", response.status);
    }
  }
  if (!terminalEvent) throw new GatewayError("模型回复没有完整结束；半截内容没有写入原文账本", response.status);
}
  ...
  private async request(input: string, init: RequestInit): Promise<Response> {
    try {
      ...
      if (cause instanceof Error && cause.name === "TimeoutError") {
        throw new GatewayError("模型服务等待超时，请检查网络后重试");
      }
      throw new GatewayError("连接不到转接服务，请检查服务是否已启动、地址是否正确");
    }
  }
